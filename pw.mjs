...
console.log('SW:', JSON.stringify(await p.evaluate(async()=>{const r=await navigator.serviceWorker.getRegistration();
...
await p.goto(B+'/jobs',{waitUntil:'networkidle'}); await p.waitForTimeout(1200);
console.log('CACHES:', JSON.stringify(await p.evaluate(async()=>{const o={};for(const n of await caches.keys())o[n]=(await (await caches.open(n)).keys()).length;return o;})));
console.log('API in cache:', await p.evaluate(async()=>{for(const n of await caches.keys())for(const r of await (await caches.open(n)).keys())if(new URL(r.url).pathname.startsWith('/api/'))return r.url;return 'none (correct)';}));
...
console.log('OFFLINE new url ->', r2?r2.status():'fail', '| fallback:', (await p.textContent('body').catch(()=>'')).includes('offline'));
